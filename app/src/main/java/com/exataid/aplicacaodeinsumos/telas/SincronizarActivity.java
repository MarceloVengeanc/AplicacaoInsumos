...
import com.exataid.aplicacaodeinsumos.telas.adapters.SincAdapter;
import com.exataid.aplicacaodeinsumos.telas.configs.ConfigWebServiceActivity;
import com.exataid.aplicacaodeinsumos.utils.OnOneOffClickListener;
import com.exataid.aplicacaodeinsumos.utils.Sync;
...
public class SincronizarActivity extends AppCompatActivity {
    ...
    private final BroadcastReceiver AtualizarDataUltimoSync = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            try {
                ...
                AIDatabase.getInstance(context).getSincDAO().inserir(s).get();
                if (listaSync.size() > 2000) {
                    AIDatabase.getInstance(context).getSincDAO().excluir(listaSync).get();
                }
                carregarBotoes();
                carregarUltimaSinc();
                carregarListener();
    ...
    private void carregarUltimaSinc() {
        ...
        try {
            sinc = AIDatabase.getInstance(this).getSincDAO().buscarAtivo().get();
            listaSync = AIDatabase.getInstance(this).getSincDAO().buscaTodos().get();
            listaURL = AIDatabase.getInstance(this).getConfigWSDAO().buscarUm().get();
            listaLogSyncCadastros = AIDatabase.getInstance(this).getLogSyncDAO().buscarTodasPorTipo("CADASTROS").get();
            listaLogSyncApontamentos = AIDatabase.getInstance(this).getLogSyncDAO().buscarTodasPorTipo("APONTAMENTOS").get();
